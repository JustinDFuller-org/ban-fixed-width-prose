...
The release workflow SHALL publish through the configured npm trusted publisher using GitHub Actions OIDC and SHALL NOT require a long-lived npm publish token stored in repository secrets.

#### Scenario: Trusted publication

- **WHEN** the release workflow reaches publication on a GitHub-hosted runner with the configured trusted publisher
...
- **THEN** CI fails the release and does not update the maintained v1 tag
